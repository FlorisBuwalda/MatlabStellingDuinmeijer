...
n = 100;                  % grid size (both dimensions)
...
while get(quit,'value') == 0
   ...
   %set boundary values
       H(1,:)=0; Hx(1,:)=0;  U(1,:)=0; %i=1 is dummy column
       H(:,1)=0; Hy(:,1)=0;  V(:,1)=0; %j=1 is a dummy row
...
%% Define Hx and Hy by dynamic upwinding depending on adjacent velocity
     ...
     %Boundaries equal to neighboring value
      ...
      %% Update U
       ...
       U(j,i) = (U(j,i)     - g*dt/dx*(H(j,i+1)-H(j,i)) ...                                                            %Gravity term
                            - dt/(2*dx)* (U(j,i+1)-U(j,i-1)).* U(j,i) ...     
                            - dt/(2*dy)* (U(j+1,i)-U(j-1,i)).*(V(j,i)+V(j-1,i)+V(j,i+1)+V(j-1,i+1))/4 )...     
                            ./(1+dt*cf*sqrt((U(j,i).^2+(V(j,i)+V(j-1,i)+V(j,i+1)+V(j-1,i+1)).^2/4))./(H(j,i)+H(j,i+1)));  %Bottom friction (implicit)
     
                        
      
      %No Central difference at boundaries                  
      j=n+3;  
      U(j,i) = (U(j,i)     - g*dt/dx*(H(j,i+1)-H(j,i)) ...                                                            
                           - dt/(2*dx)* (U(j,i+1)-U(j,i-1)).* U(j,i) ...     
                           - dt/dy*   (U(j,i)-U(j-1,i)).*(V(j-1,i)+V(j-1,i+1))/2 )...     
                           ./(1+dt*cf*sqrt((U(j,i).^2+(V(j,i)+V(j-1,i)+V(j,i+1)+V(j-1,i+1)).^2/4))./(H(j,i)+H(j,i+1))); 
                       
      j=2;  
      U(j,i) = (U(j,i)     - g*dt/dx*(H(j,i+1)-H(j,i)) ...                                                            
                           - dt/(2*dx)* (U(j,i+1)-U(j,i-1)).* U(j,i) ...     
                           - dt/dy*   (U(j+1,i)-U(j,i)).*(V(j,i)+V(j,i+1))/2 )...     
                           ./(1+dt*cf*sqrt((U(j,i).^2+(V(j,i)+V(j-1,i)+V(j,i+1)+V(j-1,i+1)).^2/4))./(H(j,i)+H(j,i+1)));                   
       %% Update V 
       i=3:n+2;
       j=2:n+2;
       ...
       V(j,i) =  (V(j,i) - g*dt/dy*(H(j+1,i)-H(j,i))  ...%                                                             %Gravity term
                         - dt/(2*dx)* (V(j,i+1)-V(j,i-1)).*(U(j,i)+U(j+1,i)+U(j,i-1)+U(j+1,i-1))/4 ...
                         - dt/(2*dy)* (V(j+1,i)-V(j-1,i)).*V(j,i)) ...  
                           ./(1+dt*cf*sqrt((U(j,i)+U(j+1,i)+U(j,i-1)+U(j+1,i-1)).^2/4+V(j,i).^2)./(H(j,i)+H(j+1,i)));     %Bottom friction (implicit)
        
       %No Central difference at boundaries                 
       i = n+3;
       V(j,i) =  (V(j,i) - g*dt/dy*(H(j+1,i)-H(j,i))   ...%                                                             
                         - dt/  dx* (V(j,i)-V(j,i-1)).*(U(j,i-1)+U(j+1,i-1))/2 ...
                         - dt/(2*dy)* (V(j+1,i)-V(j-1,i)).*V(j,i)) ...  
                           ./(1+dt*cf*sqrt((U(j,i)+U(j+1,i)+U(j,i-1)+U(j+1,i-1)).^2/4+V(j,i).^2)./(H(j,i)+H(j+1,i)));                 
       i = 2;
       V(j,i) =  (V(j,i) - g*dt/dy*(H(j+1,i)-H(j,i))   ...%                                                             
                         - dt/  dx* (V(j,i+1)-V(j,i)).*(U(j,i)+U(j+1,i))/2 ...
                         - dt/(2*dy)* (V(j+1,i)-V(j-1,i)).*V(j,i)) ...  
                           ./(1+dt*cf*sqrt((U(j,i)+U(j+1,i)+U(j,i-1)+U(j+1,i-1)).^2/4+V(j,i).^2)./(H(j,i)+H(j+1,i)));                      
    ...
    H(j,i) = H(j,i) - dt/dx*(Hx(j,i).*U(j,i)-Hx(j,i-1).*U(j,i-1))...
     ...
     %dt = .01;
      ...
      dt = min(tstep,    ...
         .5*dx/max(max(  ...    %safety factor of .3
